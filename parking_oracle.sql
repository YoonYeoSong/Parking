...
-- TABLE
DROP TABLE LOTUSER CASCADE CONSTRAINTS;
...
SELECT * FROM user_constraints WHERE table_name IN
    ('LOTUSER', 'USERHISTORY', 'CAR', 'PAYMENTHISTORY', 'REVIEW', 'QNABOARD', 'NOTICE', 'BOOKMARK', 'COUPON');


CREATE TABLE LOTUSER (
...
INSERT INTO LOTUSER VALUES('001', 'a@a.com', 'admin','010','admin', 'Seoul', DEFAULT, DEFAULT,'N','Y');
...
COMMENT ON COLUMN LOTUSER.sms_yn IS '문자 수신여부(Y/N)';
COMMENT ON COLUMN LOTUSER.email_yn IS '이메일 수신여부(Y/N)';
...
ALTER TABLE USERHISTORY
    ADD CONSTRAINT fk_userhistory_lotuser FOREIGN KEY(user_code) REFERENCES LOTUSER(user_code)
    ON DELETE CASCADE;
...
ALTER TABLE CAR
    ADD CONSTRAINT fk_car_lotuser FOREIGN KEY(user_code) REFERENCES LOTUSER(user_code)
    ON DELETE CASCADE;
...
ALTER TABLE QNABOARD
    ADD CONSTRAINT fk_qnaboard_lotuser FOREIGN KEY(user_code) REFERENCES LOTUSER(user_code)
    ON DELETE CASCADE;
...
ALTER TABLE NOTICE
    ADD CONSTRAINT fk_notice_lotuser FOREIGN KEY(user_code) REFERENCES LOTUSER(user_code)
    ON DELETE CASCADE;
...
ALTER TABLE BOOKMARK
    ADD CONSTRAINT fk_bookmark_lotuser FOREIGN KEY(user_code) REFERENCES LOTUSER(user_code)
    ON DELETE CASCADE;
...
ALTER TABLE COUPON
    ADD CONSTRAINT fk_coupon_lotuser FOREIGN KEY(user_code) REFERENCES LOTUSER(user_code)
    ON DELETE CASCADE;
ALTER TABLE COUPON
